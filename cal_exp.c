...
void FAIL_EXIT(int line_number, const char *func_name, char *msg)
{
...
void Queue_Initialize(element_queue *qq)
{
	assert(qq);	/* Make sure qq is not null */
	qq->front = qq->rear = 0;
}

int Queue_Size(element_queue *qq)
{
	assert(qq);
	return (qq->rear + MAX_TERM - qq->front) % MAX_TERM;
}

element *Queue_Insert(element_queue *qq, element *ee)
{
	int next;
	assert(qq);
	assert(ee);
...
element *Queue_Delete(element_queue *qq, element *ee)
{
	assert(qq);
	assert(ee);
...
void Stack_Initialize(element_stack *ss)
{
	assert(ss);
	ss->top = -1;
}
...
int Stack_Size(element_stack *ss)
{
	assert(ss);
	return ss->top + 1;
}

element *Push(element_stack *ss, element *ee)
{
	assert(ss);
	assert(ee);
	JWD2(("Push(%d) ", ss->top));
...
element *Pop(element_stack *ss, element *ee)
{
	assert(ss);
	assert(ee);
	JWD2(("Pop(%d) ", ss->top));
...
void Show_Queue(element_queue *qq, char *name)
{
	int ii, size;
	assert(qq);
...
void Show_Stack(element_stack *ss, char *name)
{
	int ii;
	assert(ss);
...
int	ToPostfix(element_queue *infix, element_queue *postfix)
{
	element_stack	stack;
	element	ee;
	...
	{
		SHOW_QUEUE(infix);
		if (ee.token_type != TOKEN_OPERATOR)    //耞 计or才腹 
		{
		...
		else 
		{
			while (Stack_Size(&stack)) 
			{
				...
				{
					if(ee.token_value == OPERATOR_RPAREN)
					{
						element e1;
						while(STACK_TOP(&stack).token_value != OPERATOR_LPAREN){
							Pop(&stack, &e1);
							Queue_Insert(postfix, &e1);
						}; 
						Pop(&stack, &e1);
					}
					else	break;
				...
				{
					element e2;
					Pop(&stack, &e2);	// Never empty
					if (! Queue_Insert(postfix, &e2)) 
					{
						return -1;
					}
...
int	Compute(int op, int aa, int bb, int *value)
{
	switch (op) {
...
int Evaluate(element_queue *postfix, int *ret)
{
	element_stack stack;
	element	ee;
...
/* Token Analysis */
int	analysis_token(char *token, int *token_value)
{
	char *ch;
	JWD1(("Token = %s\n", token));
	if (isdigit(token[0])) {
...
int	Parse(char *buf, element_queue *infix)
{
	char *token;
	element ee;
